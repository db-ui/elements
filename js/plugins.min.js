(function(global,factory){typeof exports==="object"&&typeof module!=="undefined"?module.exports=factory():typeof define==="function"&&define.amd?define(factory):(global=global||self,global.dialogPolyfill=factory())})(this,function(){"use strict";// nb. This is for IE10 and lower _only_.
...
dialogPolyfill.useValue=ev.offsetX+","+ev.offsetY}var dialog=findNearestDialog(target);if(!dialog){return}dialogPolyfill.formSubmitter=target},false);/**
     ...
     */var nativeFormSubmit=HTMLFormElement.prototype.submit;var replacementFormSubmit=function(){if(!isFormMethodDialog(this)){return nativeFormSubmit.call(this)}var dialog=findNearestDialog(this);dialog&&dialog.close()};HTMLFormElement.prototype.submit=replacementFormSubmit;/**
     * Global form 'dialog' method handler. Closes a dialog correctly on submit
     * and possibly sets its return value.
     */document.addEventListener("submit",function(ev){if(ev.defaultPrevented){return}// e.g. a submit which prevents default submission
var form=/** @type {HTMLFormElement} */ev.target;if(!isFormMethodDialog(form)){return}ev.preventDefault();var dialog=findNearestDialog(form);if(!dialog){return}// Forms can only be submitted via .submit() or a click (?), but anyway: sanity-check that
// the submitter is correct before using its value as .returnValue.
var s=dialogPolyfill.formSubmitter;if(s&&s.form===form){dialog.close(dialogPolyfill.useValue||s.value)}else{dialog.close()}dialogPolyfill.formSubmitter=null},false)}return dialogPolyfill});
/**
