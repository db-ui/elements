...
try {
  ...
  const cypressIntegrationsFilePaths = getAllFiles('./cypress/integration');
  ...
  if (noTestComponents.length === 0) {
    ...
    console.log(
      `Cypress tests are missing for ${JSON.stringify(
        noTestComponents
      )}. Please add them to ./cypress/integration or into the excludeComponents array inside ./scripts/cypress-component-check.js.`
    );
    process.exit(1);
  }
} catch (e) {
